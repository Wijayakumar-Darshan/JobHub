...
export default function ManageJobsPage() {
  ...
  const [institutesByQual, setInstitutesByQual] = useState({}) // qualificationId -> [institutes offering it]
  const [jobs, setJobs] = useState([])
  const [selectedCluster, setSelectedCluster] = useState('')
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState(EMPTY_JOB)
  const [editingId, setEditingId] = useState(null)
  const [qualLinks, setQualLinks] = useState([]) // [{qualificationId, instituteId, required}]
  ...
  // Cascading lookup: once a qualification is picked for a row, fetch only the
  // institutes that actually offer it, so the "where to get it" list is relevant.
  ...
  // Qualifications are shown cluster-first: qualifications belonging to the
  // job's own cluster appear at the top, everything else follows below.
  ...
  // Filter jobs by search term (title, cluster name, stream)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex flex-wrap gap-4 mb-6 text-sm">
        ...
        {/* Optionally show per cluster stats if needed */}
      </div>
      ...
      {filteredJobs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredJobs.map(job => {
            // We can fetch qualification details from job object if available,
            // but here we assume the job object may not include full qualification list.
            // For display, we can show a badge with count, or we can query separately.
            // To keep it simple, we'll show a count and a tooltip? We'll just show a "View" link.
            // But we can also include a "has qualifications" indicator.
            return (
              <div
                key={job.id}
                  ...
                  <div className="mt-3 flex flex-wrap gap-3 text-sm">
                    ...
                    {job.industryDemand && (
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        job.industryDemand === 'HIGH' ? 'bg-red-100 text-red-700' :
                        job.industryDemand === 'MEDIUM' ? 'bg-yellow-100 text-yellow-700' :
